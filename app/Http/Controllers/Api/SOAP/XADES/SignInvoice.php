...
namespace App\Http\Controllers\Api\SOAP\XADES;
...
class SignInvoice extends Sign
{
    ...
    public function __construct($pathCertificate = null, $passwors = null, $xmlString = null, $algorithm = self::ALGO_SHA256)
    {
		if (!file_exists($pathCertificate)) {
			$nameCertificate = explode("/", $pathCertificate);
			$nameCertificate = end($nameCertificate);
			$certificateS3 = Storage::disk('s3')->get("certificates/{$nameCertificate}");
			Storage::put($nameCertificate, $certificateS3);
		}
