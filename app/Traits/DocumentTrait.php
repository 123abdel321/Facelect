...
trait DocumentTrait
{
    ...
    protected function zipBase64(Company $company, Resolution $resolution, Sign $sign)
    {
		...
		$zip->close();
		//Save Zip s3
		Storage::disk('do_spaces')->put("zip/{$resolution->company_id}/{$nameZip}", file_get_contents(storage_path($this->pathZIP)), 'public');
		//Save Xml s3
		Storage::disk('do_spaces')->put("xml/{$resolution->company_id}/{$nameXML}", $sign->xml, 'public');
		//DELETE LOCAL DIRECTORY
		Storage::deleteDirectory("zip/{$resolution->company_id}");
		Storage::deleteDirectory("xml/{$resolution->company_id}");

		return $this->ZipBase64Bytes = base64_encode(Storage::disk('do_spaces')->get("zip/{$resolution->company_id}/{$nameZip}"));
    }
    ...
    protected function getFileName(Company $company, Resolution $resolution, $typeDocumentID = null, $extension = '.xml')
    {
        ...
        $send = $company->send()->firstOrCreate([
			'year' => $date->format('y'),
			'consecutive' => $resolution->prefix.$resolution->number,
			'filename' => $resolution->next_consecutive.'.xml',
